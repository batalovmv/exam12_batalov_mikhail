...
async function findDataSource(context) {
    ...
    for(let i = 0; i < files.length; i++){
        ...
        if (info) {
            const fileExports = await locter.load(info);
...
(function(EnvironmentVariableName) {
    ...
    EnvironmentVariableName[// Seeder
    "SEEDS"] = 'DB_SEEDS';
...
async function prepareSeederSeeds(input, root) {
    ...
    if (seedFiles.length > 0) {
        ...
        for(let i = 0; i < seedFiles.length; i++){
            ...
            const moduleDefault = locter.getModuleExport(moduleExports);
...
function isPromise(p) {
    return typeof p === 'object' && p !== null && (p instanceof Promise || // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
